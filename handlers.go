...
func CreateLesson(cc *ChatCompletion) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.JSON(http.StatusOK, LessonCreateResponse{
			LessonID: lessonID,
		})
	}
}
...
func ChatHistory(store *InMemoryStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		sessionID := c.Get("sessionID").(string)
		lessonName, err := store.GetLessonNameForSession(sessionID)
		if err != nil {
...
func GetPresets (store *PresetStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		presets, err := store.GetPresets()
		if err != nil {
			return echo.NewHTTPError(http.StatusInternalServerError, "error getting presets")
		}
